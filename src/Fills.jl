...
-(order::O, fill::F) where {O<:AlgorithmicMarketOrder,F<:Fill{O}} = AlgorithmicMarketOrder(order.id, order.instrument, order.action, order.side, order.size - fill.size, order.algorithm, order.timestamp)
-(order::O, fill::F) where {O<:AlgorithmicLimitOrder,F<:Fill{O}} = AlgorithmicLimitOrder(order.id, order.instrument, order.action, order.side, order.size - fill.size, order.limit, order.algorithm, order.timestamp)
